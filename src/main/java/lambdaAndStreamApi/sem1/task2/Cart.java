...
public class Cart<T extends Food> {
    ...
    public void cardBalancingV2()
    {
        ...
        boolean newProteins = foodstuffs.stream().anyMatch(Food::getProteins);
        boolean newFats = foodstuffs.stream().anyMatch(Food::getFats);
        boolean newCarbohydrates = foodstuffs.stream().anyMatch(Food::getCarbohydrates);

        if (newProteins && newFats && newCarbohydrates)
            System.out.println("Корзина сбалансирована по БЖУ.");
        else
            System.out.println("Невозможно сбалансировать корзину по БЖУ.");
    }
